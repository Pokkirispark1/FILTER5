...
from info import ADMINS, THREE_VERIFY_GAP, LOG_CHANNEL, USERNAME, VERIFY_IMG, IS_VERIFY, AUTH_CHANNEL, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, SHORTENER_API3, SHORTENER_WEBSITE3, LOG_API_CHANNEL, TWO_VERIFY_GAP, TUTORIAL, TUTORIAL2, TUTORIAL3, QR_CODE, DELETE_TIME, RFSUB_NOTIFICATION 
...
async def start(client: Client, message):
    ...
    if user_id in ADMINS:
        ...
        if not files_:
            await message.reply('<b>⚠️ ꜰɪʟᴇ ɴᴏᴛ ꜰᴏᴜɴᴅ ⚠️</b>')
            return
        files = files_[0]
    ...
    if await fsub_db.find_join_req(user_id):
        ...
        files_ = await get_file_details(file_id)
        if not files_:
            await message.reply('<b>⚠️ ɴᴏᴛ ꜰᴏᴜɴᴅ ⚠️</b>')
            return
        files = files_[0]
        CAPTION = settings.get('caption', '')
        f_caption = CAPTION.format(
    ...
    #await asyncio.sleep(600)
    #await d.delete()
    #r = await message.reply_text("<b>⚠️ ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛᴇᴅ ꜰɪʟᴇ ɪs ᴅᴇʟᴇᴛᴇᴅ ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪɴ ʙᴏᴛ, ɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴀɢᴀɪɴ ᴛʜᴇɴ sᴇᴀʀᴄʜ ᴀɢᴀɪɴ ☺️</b>")
    #await asyncio.sleep(120)
    #await r.delete()
