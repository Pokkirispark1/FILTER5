...
async def pm_search(client, message):
    if PM_SEARCH:
        ...
        mentn = message.from_user.mention
        btn = [[
            InlineKeyboardButton("🎬 𝙶𝚛𝚘𝚞𝚙 𝟷", url='https://t.me/+NYDsPNzuu_thZGRl'),
            InlineKeyboardButton("🎬 𝙶𝚛𝚘𝚞𝚙 𝟸", url='https://t.me/+Vw364TMZTkpjNWVl')
        ]]
        reply_markup = InlineKeyboardMarkup(btn)
        return await message.reply_text(
            f"<b>ʜᴇʏ {mentn} 😇</b>\n\n"
            f"ᴅᴜᴇ ᴛᴏ ᴀɴ ɪɴᴄʀᴇᴀsᴇᴅ ɴᴜᴍʙᴇʀ ᴏғ ʀᴇǫᴜᴇsᴛs, ᴡᴇ ᴀʀᴇ ғᴏʀᴄᴇᴅ ᴛᴏ ᴅɪsᴀʙʟᴇ ᴘᴍ sᴇᴀʀᴄʜ.\n\n"
            f"ʙᴜᴛ ᴡᴏʀʀʏ ɴᴏᴛ, ᴀ ɴᴇᴡ ʙᴏᴛ ɪs ᴏɴ ᴛʜᴇ ᴡᴀʏ ᴡʜɪᴄʜ sᴜᴘᴘᴏʀᴛs ʀᴇǫᴜᴇsᴛs ɪɴ ᴘᴍ.\n\n"
            f"ғᴏʀ ɴᴏᴡ, ʏᴏᴜ ᴄᴀɴ ʀᴇǫᴜᴇsᴛ ᴍᴏᴠɪᴇs ᴏʀ sᴇʀɪᴇs ғʀᴏᴍ ᴏᴜʀ ɢʀᴏᴜᴘs 👇",
...
async def group_search(client, message):
    ...
    if ALLOWED_GROUP_IDS and chat_id not in ALLOWED_GROUP_IDS:
        mentn = message.from_user.mention if message.from_user else "User"
        chat_name = message.chat.title if message.chat.title else "this group"
        ...
        await message.reply_text(
            f"<b>ʜᴇʏ {mentn} 😇 thanks for adding me in {chat_name}</b>\n\n"
            f"ᴅᴜᴇ ᴛᴏ ᴀɴ ɪɴᴄʀᴇᴀsᴇᴅ ɴᴜᴍʙᴇʀ ᴏғ ʀᴇǫᴜᴇsᴛs, ᴡᴇ ᴀʀᴇ ғᴏʀᴄᴇᴅ ᴛᴏ ᴅɪsᴀʙʟᴇ sᴇᴀʀᴄʜ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ.\n\n"
            f"ʙᴜᴛ ᴡᴏʀʀʏ ɴᴏᴛ, ᴀ ɴᴇᴡ ʙᴏᴛ ɪs ᴏɴ ᴛʜᴇ ᴡᴀʏ ᴡʜɪᴄʜ sᴜᴘᴘᴏʀᴛs ʀᴇǫᴜᴇsᴛs ɪɴ ᴏᴛʜᴇʀ ɢʀᴏᴜᴘs.\n\n"
            f"ғᴏʀ ɴᴏᴡ, ʏᴏᴜ ᴄᴀɴ ʀᴇǫᴜᴇsᴛ ᴍᴏᴠɪᴇs ᴏʀ sᴇʀɪᴇs ғʀᴏᴍ ᴏᴜʀ ɢʀᴏᴜᴘs 👇",
            reply_markup=reply_markup
        )
        return
...
async def next_page(bot, query):
    ...
    del_msg = f"\n\n<b><blockquote>Jᴏɪɴ @CM_Zone Fᴏʀ Uᴘᴅᴀᴛᴇs ❄️</blockquote></b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
        ...
        btn = [[InlineKeyboardButton(text=f"🗃️ {get_size(file.file_size)}▪️{get_name(file.file_name)}", url=f'https://telegram.dog/{temp.U_NAME}?start=file_{query.message.chat.id}_{file.file_id}'),]
                for file in files
              ]
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    if settings["link"]:
        ...
        btn = [[
            InlineKeyboardButton(text=f"🗃️ {get_size(file.file_size)}▪️{get_name(file.file_name)}", url=f'https://t.me/{temp.U_NAME}?start=file_{reqnxt}_{file.file_id}')]
               for file in files
              ]
    if not settings["is_verify"]:
        btn.insert(0,[
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        message = msg
        #search = message.text
        search = re.sub(r'[^A-Za-z0-9\s]', '', message.text.lower().replace('\n', ' '))
        search = re.sub(r"\b(season\d{1,2}|episode\d{1,2}|season\s\d{1,2}|episode\s\d{1,2}|s\d{1,2}|ep\d{1,2}|e\d{1,2}|s\s\d{1,2}|ep\s\d{1,2}|e\s\d{1,2})\b", "", search).strip()
        chat_id = message.chat.id
        settings = await get_settings(chat_id)
        files, offset, total_results = await get_search_results(search)
        if not files:
    ...
    if settings["link"]:
        ...
        btn = [[InlineKeyboardButton(text=f"🗃️ {get_size(file.file_size)}▪️{get_name(file.file_name)}", url=f'https://telegram.dog/{temp.U_NAME}?start=file_{message.chat.id}_{file.file_id}'),]
               for file in files
              ]
